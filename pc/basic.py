"""
basic.py — Single run smoke test for HLS sweep pipeline
========================================================
Creates a self-contained HLS component under enee759u/basic/
with its own fir.cpp and hls_config.cfg.
Shares fir_tb.cpp, taps_init.h, and io/ from enee759u/ level.
Never touches the original fir/ component.

DIRECTORY STRUCTURE (run from enee759u/):
------------------------------------------
enee759u/
├── basic.py               (this file)
├── sweep.py
...
├── io/
│   ├── input.mem          (shared)
│   └── output_15.mem      (shared)
├── fir/                   (original component, untouched)
│   ├── fir.cpp
│   ├── hls_config.cfg
│   └── fir/hls/syn/report/
└── basic/                 (created by this script)
    ├── fir.cpp            (generated, overwritten each run)
    ├── hls_config.cfg     (generated, overwritten each run)
    └── basic/hls/syn/report/

HOW TO RUN:
-----------
  Option A — plain Windows CMD or PowerShell (recommended):
      cd D:\AMDDesignTools\enee759u
      python basic.py

  Option B — Git Bash (also fine):
      cd /d/AMDDesignTools/enee759u
      python basic.py

  Do NOT source Vitis settings64.sh inside Git Bash and then call python
  from a CMD session; that creates the mangled "D;D:\..." PATH entries.
"""
